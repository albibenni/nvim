...
function M.schedule_task()
	...
	vim.ui.input({ prompt = "Todoist Task: ", default = selected_text }, function(task_content)
		...
		vim.ui.input(
			{ prompt = "Schedule for (e.g. tod, tom, 1d, 1w, 1m) [Leave empty for Inbox]: " },
			function(due_string)
				...
				vim.ui.select(priorities, {
					...
					curl.get("https://api.todoist.com/api/v1/projects", {
						...
						on_error = vim.schedule_wrap(function(err)
							vim.notify(
								"Network error fetching Todoist projects: " .. (err.message or tostring(err)),
								vim.log.levels.ERROR
							)
						end),
						callback = vim.schedule_wrap(function(res)
							...
							vim.ui.select(project_options, {
								...
								curl.post("https://api.todoist.com/api/v1/tasks", {
									...
									on_error = vim.schedule_wrap(function(err)
										vim.notify(
											"Network error creating Todoist task: " .. (err.message or tostring(err)),
											vim.log.levels.ERROR
										)
									end),
